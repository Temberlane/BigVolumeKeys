...
final class VolumeControllerTests: XCTestCase {
    ...
    func testIncreaseVolumeFromZero() async {
        ...
        XCTAssertEqual(Double(mockAudioManager.lastSetVolumeValue ?? 0), 0.05, accuracy: 0.001)
    }
    ...
    func testIncreaseVolumeFromMiddle() async {
        ...
        XCTAssertEqual(Double(mockAudioManager.lastSetVolumeValue ?? 0), 0.55, accuracy: 0.001)
    }

    func testIncreaseVolumeCapsAtMax() async {
        let device = createTestDevice(id: 1, volume: 0.98, isMuted: false)
        mockAudioManager.mockCurrentDevice = device
        mockAudioManager.mockVolumes[1] = 0.98
    ...
    func testDecreaseVolumeFromFull() async {
        ...
        XCTAssertEqual(Double(mockAudioManager.lastSetVolumeValue ?? 0), 0.95, accuracy: 0.001)
    }
    ...
    func testDecreaseVolumeFromMiddle() async {
        ...
        XCTAssertEqual(Double(mockAudioManager.lastSetVolumeValue ?? 0), 0.45, accuracy: 0.001)
    }

    func testDecreaseVolumeFloorsAtZero() async {
        let device = createTestDevice(id: 1, volume: 0.02, isMuted: false)
        mockAudioManager.mockCurrentDevice = device
        mockAudioManager.mockVolumes[1] = 0.02
    ...
    func testVolumeStepIs5Percent() async {
        XCTAssertEqual(Double(volumeController.volumeStep), 0.05, accuracy: 0.001)
    }

    func testVolumeRoundsToNearestStep() async {
        // When current volume is 0.53, increasing should round to nearest 0.05
        let device = createTestDevice(id: 1, volume: 0.53, isMuted: false)
        mockAudioManager.mockCurrentDevice = device
        mockAudioManager.mockVolumes[1] = 0.53

        volumeController.increaseVolume()

        // 0.53 + 0.05 = 0.58, rounded to 0.60
        XCTAssertEqual(Double(mockAudioManager.lastSetVolumeValue ?? 0), 0.60, accuracy: 0.001)
    }
    ...
    func testMultipleRapidVolumeIncreases() async {
        ...
        XCTAssertEqual(Double(mockAudioManager.lastSetVolumeValue ?? 0), 0.50, accuracy: 0.001)
    }

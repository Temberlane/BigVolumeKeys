...
final class BigVolumeKeysTests: XCTestCase {
    ...
    func testEndToEndVolumeControlWorkflow() async {
        ...
        // Simulate volume key presses (1% step)
        controller.increaseVolume()
        XCTAssertEqual(Double(mockManager.lastSetVolumeValue ?? 0), 0.51, accuracy: 0.01)

        mockManager.mockVolumes[1] = 0.51  // Simulate manager update before next call
    ...
    func testVolumeStepConsistency() async {
        ...
        // 100 increases at 1% each should get to 100%
        for i in 0..<100 {
            controller.increaseVolume()
            mockManager.mockVolumes[1] = mockManager.lastSetVolumeValue ?? 0

            let expectedVolume = min(1.0, Float(i + 1) * 0.01)
    ...
    func testVolumeBoundaryBehavior() async {
        ...
        let device = AudioDevice(
            id: 1,
            name: "Test",
            volume: 0.995,  // Close to max (1% step will exceed 1.0)
            isMuted: false,
            isMultiOutput: false
        )
        mockManager.mockCurrentDevice = device
        mockManager.mockVolumes[1] = 0.995
